...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs) || gArgs.IsArgSet("-version")) {
        ...
        std::string strUsage = PACKAGE_NAME " photon-tx utility version " + FormatFullVersion() + "\n";

        if (gArgs.IsArgSet("-version")) {
            strUsage += FormatParagraph(LicenseInfo());
        } else {
            strUsage += "\n"
                "Usage:  photon-tx [options] <hex-tx> [commands]  Update hex-encoded Photon transaction\n"
                "or:     photon-tx [options] -create [commands]   Create hex-encoded Photon transaction\n"
                "\n";
