...
BOOST_FIXTURE_TEST_CASE(SubtractFee, TestChain100Setup)
{
    ...
    CAmount input_amount;
    {
        ...
        const auto coins{AvailableCoins(*wallet).All()};
    ...
    auto check_tx = [&wallet, input_amount, input_outpoint](CAmount leftover_input_amount) {
        CRecipient recipient{GetScriptForRawPubKey({}), input_amount - leftover_input_amount, /*subtract_fee=*/true};
...
BOOST_FIXTURE_TEST_CASE(wallet_duplicated_preset_inputs_test, TestChain100Setup)
{
    ...
    // Add 4 spendable Photon UTXOs to the wallet.
    ...
    const CAmount oversized_payment{wallet_total + COIN};

    // Try to create a tx that spends more than what preset inputs + wallet selected inputs are covering for.
    // The wallet can cover its actual Photon total, and the target is one PHO more.
    std::vector<CRecipient> recipients = {{GetScriptForDestination(*Assert(wallet->GetNewDestination(OutputType::BECH32, "dummy"))),
                                           /*nAmount=*/oversized_payment, /*fSubtractFeeFromAmount=*/true}};
    CCoinControl coin_control;
    ...
    // Attempt to send more PHO than the wallet holds. The wallet should exclude
    // the preset inputs from the pool of available coins, realize there is not enough
    // money to fund the payment, and fail with "Insufficient funds".
    //
    // Even with SFFO, the wallet can only afford its actual Photon balance.
