...
BOOST_FIXTURE_TEST_CASE(scan_for_wallet_transactions, TestChain100Setup)
{
    ...
    const CAmount old_tip_coinbase{CoinbaseValue(m_coinbase_txns.back())};
    const CBlock new_block = CreateAndProcessBlock({}, GetScriptForRawPubKey(coinbaseKey.GetPubKey()));
    const CAmount new_tip_coinbase{CoinbaseValue(new_block.vtx[0])};
    ...
    // and new block files.
    {
        ...
        BOOST_CHECK_EQUAL(GetBalance(wallet).m_mine_immature, old_tip_coinbase + new_tip_coinbase);
    ...
    // file.
    {
        ...
        BOOST_CHECK_EQUAL(result.last_scanned_block, newTip->GetBlockHash());
        BOOST_CHECK_EQUAL(*result.last_scanned_height, newTip->nHeight);
        BOOST_CHECK_EQUAL(GetBalance(wallet).m_mine_immature, new_tip_coinbase);
    }
...
BOOST_FIXTURE_TEST_CASE(coin_mark_dirty_immature_credit, TestChain100Setup)
{
    ...
    wtx.MarkDirty();
    AddKey(wallet, coinbaseKey);
    BOOST_CHECK_EQUAL(CachedTxGetImmatureCredit(wallet, wtx, ISMINE_SPENDABLE), CoinbaseValue(m_coinbase_txns.back()));
}
...
BOOST_FIXTURE_TEST_CASE(ListCoinsTest, ListCoinsTestingSetup)
{
    ...
    BOOST_CHECK_EQUAL(CoinbaseValue(m_coinbase_txns.front()), WITH_LOCK(wallet->cs_wallet, return AvailableCoins(*wallet).GetTotalAmount()));
