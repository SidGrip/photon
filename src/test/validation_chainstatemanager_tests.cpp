...
struct SnapshotTestSetup : TestChain100Setup {
    static constexpr int SNAPSHOT_HEIGHT{COINBASE_MATURITY + 10};
    static constexpr size_t NEW_COINS{100};
    static constexpr int BACKGROUND_REWIND_HEIGHT{SNAPSHOT_HEIGHT - 1};
    static constexpr int POST_SETUP_HEIGHT{SNAPSHOT_HEIGHT + static_cast<int>(NEW_COINS)};
    static constexpr int POST_RESTART_MINE_HEIGHT{POST_SETUP_HEIGHT + 10};
    ...
    std::tuple<Chainstate*, Chainstate*> SetupSnapshot()
    {
        ...
        size_t initial_size;
        size_t initial_total_coins{COINBASE_MATURITY};
        ...
        // Mine to the Photon regtest assumeutxo height where a valid value can
        // be found.
        const int blocks_to_snapshot{SNAPSHOT_HEIGHT - static_cast<int>(initial_total_coins)};
        mineBlocks(blocks_to_snapshot);
        initial_size += blocks_to_snapshot;
        initial_total_coins += blocks_to_snapshot;
...
BOOST_FIXTURE_TEST_CASE(chainstatemanager_snapshot_init, SnapshotTestSetup)
{
    ...
    BOOST_CHECK_EQUAL(bg_chainstate.m_chain.Height(), SnapshotTestSetup::BACKGROUND_REWIND_HEIGHT);
...

    {
        ...
        BOOST_CHECK_EQUAL(chainman_restarted.ActiveHeight(), SnapshotTestSetup::POST_SETUP_HEIGHT);
    }
    ...
    {
        LOCK(chainman_restarted.GetMutex());
        BOOST_CHECK_EQUAL(chainman_restarted.ActiveHeight(), SnapshotTestSetup::POST_RESTART_MINE_HEIGHT);

        // Background chainstate should be unaware of new blocks on the snapshot
        // chainstate.
        for (Chainstate* cs : chainman_restarted.GetAll()) {
            if (cs != &chainman_restarted.ActiveChainstate()) {
                BOOST_CHECK_EQUAL(cs->m_chain.Height(), SnapshotTestSetup::BACKGROUND_REWIND_HEIGHT);
            }
        }
    }
}
