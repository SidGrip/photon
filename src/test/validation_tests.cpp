...
static unsigned int SubsidyBits(const Consensus::Params& consensusParams)
{
    return UintToArith256(consensusParams.powLimit).GetCompact();
}

static void TestPhotonSubsidy(const Consensus::Params& consensusParams)
{
    const unsigned int bits = SubsidyBits(consensusParams);
    BOOST_CHECK_EQUAL(GetBlockSubsidy(0, bits, consensusParams), 32768 * COIN);
    BOOST_CHECK(GetBlockSubsidy(1, bits, consensusParams) >= 32768 * COIN);
    BOOST_CHECK(GetBlockSubsidy(150, bits, consensusParams) >= GetBlockSubsidy(1, bits, consensusParams));
    BOOST_CHECK(GetBlockSubsidy(210000, bits, consensusParams) >= GetBlockSubsidy(150, bits, consensusParams));
}
...
BOOST_AUTO_TEST_CASE(block_subsidy_test)
{
    ...
    TestPhotonSubsidy(main_params->GetConsensus());
    TestPhotonSubsidy(testnet_params->GetConsensus());
    TestPhotonSubsidy(regtest_params->GetConsensus());
}
...
BOOST_AUTO_TEST_CASE(subsidy_limit_test)
{
    ...
    for (int nHeight = 0; nHeight < 14000000; nHeight += 1000) {
        CAmount nSubsidy = GetBlockSubsidy(nHeight, SubsidyBits(chainParams->GetConsensus()), chainParams->GetConsensus());
        BOOST_CHECK(nSubsidy >= 32768 * COIN);
        nSum += nSubsidy * 1000;
    }
    BOOST_CHECK(nSum > 0);
}
...
//! Test retrieval of valid assumeutxo values. Photon regtest carries one
//! snapshot at height 130 with a Photon-specific txoutset hash.
BOOST_AUTO_TEST_CASE(test_assumeutxo)
{
    ...
    const auto snapshot_130 = ExpectedAssumeutxo(130, *params);
    BOOST_REQUIRE(snapshot_130);
    BOOST_CHECK_EQUAL(
        snapshot_130->hash_serialized.ToString(),
        "2657be9892a9c85dc17b94beb739acaf74e92b777d6e46910e807451b75af428");
    BOOST_CHECK_EQUAL(snapshot_130->nChainTx, 131U);

    for (const int empty : {0, 100, 110, 120, 129, 131, 200, 229, 231}) {
    ...
    const auto main_snapshot = ExpectedAssumeutxo(2057519, *main_params);
    BOOST_REQUIRE(main_snapshot);
    BOOST_CHECK_EQUAL(
        main_snapshot->hash_serialized.ToString(),
        "b76486b1888d365bedb9ed3009fcb1da7184c196e54fe71bd4a74a13030a61a6");
    BOOST_CHECK_EQUAL(main_snapshot->nChainTx, 3566180U);
}
...
BOOST_AUTO_TEST_CASE(block_malleation)
{
...

    {
        ...
        uint256 merkle_collision_candidate;
        {
            ...
            // tree inner nodes using the same hash function. Blakecoin txids are
        ...
        // reproduces the original merkle root under Blakecoin's hash policy.
        block.vtx.clear();
