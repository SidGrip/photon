...
BOOST_AUTO_TEST_CASE(findBlock)
{
    ...
    uint256 next_hash;
    const int active_height{active.Height()};
    BOOST_CHECK_EQUAL(active_height, COINBASE_MATURITY);
    BOOST_CHECK(chain->findBlock(active[active_height - 1]->GetBlockHash(), FoundBlock().inActiveChain(cur_active).nextBlock(FoundBlock().inActiveChain(next_active).hash(next_hash))));
    BOOST_CHECK(cur_active);
    BOOST_CHECK(next_active);
    BOOST_CHECK_EQUAL(next_hash, active[active_height]->GetBlockHash());
    cur_active = next_active = false;
    BOOST_CHECK(chain->findBlock(active[active_height]->GetBlockHash(), FoundBlock().inActiveChain(cur_active).nextBlock(FoundBlock().inActiveChain(next_active))));
    BOOST_CHECK(cur_active);
    BOOST_CHECK(!next_active);
