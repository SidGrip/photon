...
static CAmount CoinbaseValue(const CTransactionRef& tx)
{
    return tx->vout.at(0).nValue;
}
...
BOOST_FIXTURE_TEST_CASE(package_validation_tests, TestChain100Setup)
{
    ...
    const CAmount parent_value{CoinbaseValue(m_coinbase_txns[0]) - COIN};
    ...
    auto mtx_child = CreateValidMempoolTransaction(/*input_transaction=*/tx_parent, /*input_vout=*/0,
                                                   ...
                                                   /*output_amount=*/parent_value - COIN, /*submit=*/false);
...
BOOST_FIXTURE_TEST_CASE(package_submission_tests, TestChain100Setup)
{
    ...
    for (size_t i{0}; i < 10; ++i) {
        auto mtx = CreateValidMempoolTransaction(/*input_transaction=*/m_coinbase_txns[i + 25], /*input_vout=*/0,
                                                 /*input_height=*/0, /*input_signing_key=*/coinbaseKey,
                                                 /*output_destination=*/parent_locking_script,
                                                 /*output_amount=*/CoinbaseValue(m_coinbase_txns[i + 25]) - COIN, /*submit=*/false);
    ...
    auto mtx_grandchild = CreateValidMempoolTransaction(/*input_transaction=*/tx_child, /*input_vout=*/0,
                                                       ...
                                                       /*output_amount=*/parent_value - 2 * COIN, /*submit=*/false);
...
BOOST_FIXTURE_TEST_CASE(package_witness_swap_tests, TestChain100Setup)
{
    ...
    mtx_child1.vout[0].nValue = parent_value - COIN;
    ...
    const CAmount parent1_value{CoinbaseValue(m_coinbase_txns[1]) - COIN};
    ...
    const CAmount grandparent2_value{CoinbaseValue(m_coinbase_txns[2]) - COIN};
    ...
    mtx_parent2_v1.vout[0].nValue = grandparent2_value - COIN;
    ...
    const CAmount parent3_value{CoinbaseValue(m_coinbase_txns[3])};
    ...
    mtx_mixed_child.vout.push_back(CTxOut(parent1_value + (grandparent2_value - COIN) + parent3_value - COIN, mixed_child_spk));
...
BOOST_FIXTURE_TEST_CASE(package_cpfp_tests, TestChain100Setup)
{
    ...
    const CAmount coinbase_value{CoinbaseValue(m_coinbase_txns[0])};
