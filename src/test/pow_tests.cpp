...
BOOST_AUTO_TEST_CASE(get_next_work_uses_full_interval_after_150000)
{
    const auto chainParams = CreateChainParams(*m_node.args, CBaseChainParams::MAIN);
    const auto& consensus = chainParams->GetConsensus();
    const int interval = consensus.DifficultyAdjustmentInterval();
    std::vector<CBlockIndex> blocks(interval + 1);
    const int first_height = 150000 - interval - 1;
    const int64_t first_time = 5'000'000;
    const int64_t tip_time = first_time + consensus.nPowTargetTimespan;
    const arith_uint256 old_target = UintToArith256(consensus.powLimit) / 8;
    const uint32_t old_nbits = CompactTarget(old_target);

    for (int i = 0; i <= interval; ++i) {
        blocks[i].nHeight = first_height + i;
        blocks[i].nTime = first_time + consensus.nPowTargetTimespan / 2;
        blocks[i].nBits = old_nbits;
        blocks[i].pprev = i == 0 ? nullptr : &blocks[i - 1];
    }
    blocks.front().nTime = first_time;
    blocks.back().nTime = tip_time;

    CBlockHeader next_block;
    next_block.nTime = tip_time + consensus.nPowTargetSpacing;
    BOOST_CHECK_EQUAL(GetNextWorkRequired(&blocks.back(), &next_block, consensus), old_nbits);
}
...
BOOST_AUTO_TEST_CASE(get_next_work_lower_limit_actual)
{
    ...
    const int64_t limited_timespan = consensus.nPowTargetTimespan * 100 / 115;
    arith_uint256 expected_target = old_target;
    expected_target *= limited_timespan;
    expected_target /= consensus.nPowTargetTimespan;
    const uint32_t expected_nbits = CompactTarget(expected_target);
    BOOST_CHECK_EQUAL(CalculateNextWorkRequired(&pindexLast, nLastRetargetTime, consensus), expected_nbits);
    BOOST_CHECK(PermittedDifficultyTransition(consensus, pindexLast.nHeight + 1, pindexLast.nBits, expected_nbits));

    const uint32_t invalid_nbits = CompactTarget(expected_target / 2);
...
BOOST_AUTO_TEST_CASE(get_next_work_lower_limit_actual_post_3500)
{
    ...
    CBlockIndex pindexLast;
    const int post_3500_adjustment_height = ((3500 / consensus.DifficultyAdjustmentInterval()) + 1) * consensus.DifficultyAdjustmentInterval();
    pindexLast.nHeight = post_3500_adjustment_height - 1;
    pindexLast.nTime = nLastRetargetTime + consensus.nPowTargetTimespan / 10;
    const arith_uint256 old_target = UintToArith256(consensus.powLimit) / 8;
    pindexLast.nBits = CompactTarget(old_target);

    const int64_t limited_timespan = consensus.nPowTargetTimespan * 100 / 103;
    arith_uint256 expected_target = old_target;
    expected_target *= limited_timespan;
    expected_target /= consensus.nPowTargetTimespan;
    const uint32_t expected_nbits = CompactTarget(expected_target);
    ...
    const uint32_t quarter_timespan_nbits = CompactTarget(old_target / 4);
    BOOST_CHECK(PermittedDifficultyTransition(consensus, pindexLast.nHeight + 1, pindexLast.nBits, quarter_timespan_nbits));

    const uint32_t invalid_nbits = CompactTarget(old_target / 5);
    BOOST_CHECK(!PermittedDifficultyTransition(consensus, pindexLast.nHeight + 1, pindexLast.nBits, invalid_nbits));
}
...
BOOST_AUTO_TEST_CASE(get_next_work_upper_limit_actual)
{
    ...
    const arith_uint256 expected_target = old_target * 2;
    const uint32_t expected_nbits = CompactTarget(expected_target);
    BOOST_CHECK_EQUAL(CalculateNextWorkRequired(&pindexLast, nLastRetargetTime, consensus), expected_nbits);
    BOOST_CHECK(PermittedDifficultyTransition(consensus, pindexLast.nHeight + 1, pindexLast.nBits, expected_nbits));
