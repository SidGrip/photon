...
constexpr static unsigned char BLOCK_EXTRANONCES[]{
    ...
    1, 4, 1, 4, 2, 1, 3, 5, 6, 2, 3, 7, 3, 0, 9, 8, 5, 0, 8, 9,
};
...
void MinerTestingSetup::TestBasicMining(const CScript& scriptPubKey, const std::vector<CTransactionRef>& txFirst, int baseheight)
{
    ...
    // Photon 0.25.2 enforces relative locks during block assembly here, so
    ...
    // The extra height keeps the absolute and relative lock checks aligned under Photon maturity.
    for (int i = 0; i < CBlockIndex::nMedianTimeSpan; ++i) {
        CBlockIndex* ancestor{Assert(m_node.chainman->ActiveChain().Tip()->GetAncestor(m_node.chainman->ActiveChain().Tip()->nHeight - i))};
...
BOOST_AUTO_TEST_CASE(CreateNewBlock_validity)
{
    ...
    // Keep ten spare blocks beyond maturity for the sequence-lock subtests.
    static_assert(std::size(BLOCK_EXTRANONCES) == static_cast<size_t>(COINBASE_MATURITY + 10), "Should have enough blocks to import");
    int baseheight = 0;
    ...
    for (const auto extranonce : BLOCK_EXTRANONCES) {
        ...
        CBlock *pblock = &pblocktemplate->block; // pointer for convenience
        {
            ...
            // Photon regtest starts from the historical genesis nBits. Push
