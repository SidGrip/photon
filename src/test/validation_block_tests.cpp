...
std::shared_ptr<CBlock> MinerTestingSetup::Block(const uint256& prev_hash)
{
    ...
    // grinding. On Blakecoin regtest, step synthetic blocks past the
    ...
    // Photon's block subsidy is height-dependent. This helper may retarget the
    // template onto a fork by replacing hashPrevBlock, so derive the synthetic
    // coinbase value from the selected previous block instead of the active tip.
    txCoinbase.vout[1].nValue = GetBlockSubsidy(prev_block->nHeight + 1, pblock->nBits, Params().GetConsensus());
