...
bool CoinStatsIndex::CustomAppend(const interfaces::BlockInfo& block)
{
    ...
    const CBlockIndex* pindex = block.height > 0 ? WITH_LOCK(cs_main, return m_chainstate->m_blockman.LookupBlockIndex(block.hash)) : nullptr;
    const unsigned int nBits = pindex ? pindex->nBits : (block.data ? block.data->nBits : Params().GenesisBlock().nBits);
    const CAmount block_subsidy{GetBlockSubsidy(block.height, nBits, Params().GetConsensus())};
...
bool CoinStatsIndex::ReverseBlock(const CBlock& block, const CBlockIndex* pindex)
{
    ...
    const CAmount block_subsidy{GetBlockSubsidy(pindex->nHeight, pindex->nBits, Params().GetConsensus())};
    m_total_subsidy -= block_subsidy;
