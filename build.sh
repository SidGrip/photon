...
#
# Repository: https://github.com/SidGrip/Photon.git (branch: photon-25.2-auxpow-port)
# =============================================================================
...
DAEMON_NAME="photond"
QT_NAME="photon-qt"
QT_WM_CLASS="Photon-Qt"
QT_DESKTOP_ID="org.photon.photon-qt"
CLI_NAME="photon-cli"
TX_NAME="photon-tx"
...
\`\`\`bash
cp photond photon-cli photon-tx photon-wallet photon-util ~/.local/bin/
\`\`\`
...
DEOF
    ...
    info "Desktop launcher installed  -  Photon will appear in Activities search"
}
...
if is_enabled "$BLAKE_ENABLE_ZMQ"; then
    if [ -f "${MXE_SYSROOT}/lib/pkgconfig/libzmq.pc" ] && ! pkg-config --atleast-version=4 libzmq 2>/dev/null; then
        echo ">>> Normalizing MXE libzmq pkg-config version for Bitcoin Core configure..."
...
# matching pkg-config files. Bitcoin Core Qt m4 probes the .pc names before
...
${HOST}-strip src/photon-wallet.exe 2>/dev/null || true
${HOST}-strip src/photon-util.exe 2>/dev/null || true

echo ">>> Build complete!"
ls -lh src/photond.exe src/qt/photon-qt.exe src/photon-cli.exe src/photon-tx.exe src/photon-wallet.exe src/photon-util.exe 2>/dev/null || true
'
    ...
    if [[ "$target" == "daemon" || "$target" == "both" ]]; then
        ...
        docker cp "$container_name:/build/blakecoin/src/photond.exe" "$output_dir/photond-${VERSION}.exe" 2>/dev/null || true
        docker cp "$container_name:/build/blakecoin/src/photon-cli.exe" "$output_dir/photon-cli-${VERSION}.exe" 2>/dev/null || true
        docker cp "$container_name:/build/blakecoin/src/photon-tx.exe" "$output_dir/photon-tx-${VERSION}.exe" 2>/dev/null || true
        docker cp "$container_name:/build/blakecoin/src/photon-wallet.exe" "$output_dir/photon-wallet-${VERSION}.exe" 2>/dev/null || true
        docker cp "$container_name:/build/blakecoin/src/photon-util.exe" "$output_dir/photon-util-${VERSION}.exe" 2>/dev/null || true
    fi

    if [[ "$target" == "qt" || "$target" == "both" ]]; then
        info "Extracting Qt wallet..."
        docker cp "$container_name:/build/blakecoin/src/qt/photon-qt.exe" "$output_dir/photon-qt-${VERSION}.exe" 2>/dev/null || true
    fi
...
'
    ...
    if [[ "$target" == "daemon" || "$target" == "both" ]]; then
        info "Extracting daemon binaries..."
        docker cp "$container_name:/build/blakecoin/src/photond" "$output_dir/photond-${VERSION}" 2>/dev/null || true
        docker cp "$container_name:/build/blakecoin/src/photon-cli" "$output_dir/photon-cli-${VERSION}" 2>/dev/null || true
        docker cp "$container_name:/build/blakecoin/src/photon-tx" "$output_dir/photon-tx-${VERSION}" 2>/dev/null || true
    fi
    ...
    if [[ "$target" == "qt" || "$target" == "both" ]]; then
        ...
        if docker cp "$container_name:/build/blakecoin/$app_name" "$output_dir/$app_name" 2>/dev/null; then
            ...
            docker exec "$container_name" find /build/blakecoin -name "*.app" -type d 2>/dev/null || true
        fi
        # Also copy raw binary for convenience
        docker cp "$container_name:/build/blakecoin/src/qt/photon-qt" "$output_dir/photon-qt-${VERSION}" 2>/dev/null || true
    fi
...
build_macos_cross() {
    ...
    docker create \
        ...
        -v "$tmpdir:/build/blakecoin:rw" \
        -v "$cache_root/macos-depends-built:/build/blakecoin/depends/built:rw" \
        -v "$cache_root/macos-depends-sources:/build/blakecoin/depends/sources:rw" \
        "$DOCKER_MACOS" \
        /bin/bash -lc '
set -euo pipefail
cd /build/blakecoin
...
# Bitcoin Core 25.2 depends still uses the prebuilt LLVM 10 native_clang
...
if [[ "${MACOS_FORCE_SYSTEM_CLANG:-1}" == "1" ]]; then
    ...
    echo ">>> Using Bitcoin Core-style depends-managed clang/cctools for macOS."
fi
...
if [[ "$BLAKE_TARGET" == "qt" || "$BLAKE_TARGET" == "both" ]]; then
    if [[ -f share/qt/Info.plist ]]; then
        sed -i \
            ...
            -e "s/application\/x-bitcoin-payment-request/application\/x-photon-payment-request/g" \
...
' >/dev/null
    ...
    if [[ "$target" == "qt" || "$target" == "both" ]]; then
        ...
        if docker cp "$container_name:/build/blakecoin/Photon-Qt.app" "$output_dir/Photon-Qt.app" 2>/dev/null; then
            ...
            docker exec "$container_name" find /build/blakecoin -maxdepth 2 -name "*.app" -type d 2>/dev/null || true
        fi
        docker cp "$container_name:/build/blakecoin/src/qt/photon-qt" "$output_dir/photon-qt-${VERSION}" 2>/dev/null || true
    fi
...
strip src/photon-wallet 2>/dev/null || true
strip src/photon-util 2>/dev/null || true

echo ">>> Build complete!"
ls -lh src/photond src/qt/photon-qt src/photon-cli src/photon-tx src/photon-wallet src/photon-util 2>/dev/null || true
'
    ...
    if [[ "$target" == "daemon" || "$target" == "both" ]]; then
        info "Extracting daemon binaries..."
        docker cp "$container_name:/build/blakecoin/src/photond" "$daemon_stage/photond-${VERSION}" 2>/dev/null || true
        docker cp "$container_name:/build/blakecoin/src/photon-cli" "$daemon_stage/photon-cli-${VERSION}" 2>/dev/null || true
        docker cp "$container_name:/build/blakecoin/src/photon-tx" "$daemon_stage/photon-tx-${VERSION}" 2>/dev/null || true
        docker cp "$container_name:/build/blakecoin/src/photon-wallet" "$daemon_stage/photon-wallet-${VERSION}" 2>/dev/null || true
        docker cp "$container_name:/build/blakecoin/src/photon-util" "$daemon_stage/photon-util-${VERSION}" 2>/dev/null || true
    fi

    if [[ "$target" == "qt" || "$target" == "both" ]]; then
        info "Extracting Qt wallet..."
        docker cp "$container_name:/build/blakecoin/src/qt/photon-qt" "$qt_stage/photon-qt-${VERSION}" 2>/dev/null || true
    fi
    ...
    docker cp "$container_name:/build/blakecoin/test/config.ini" "$final_output_dir/test-config.ini" 2>/dev/null || warn "Missing native Docker test/config.ini"
    docker cp "$container_name:/build/blakecoin/config.log" "$final_output_dir/config.log" 2>/dev/null || warn "Missing native Docker config.log"
    if [[ "$target" == "daemon" || "$target" == "both" ]]; then
        cp "$daemon_stage/photon-wallet-${VERSION}" "$final_output_dir/$WALLET_NAME" 2>/dev/null || warn "Missing Linux wallet tool artifact: $daemon_stage/photon-wallet-${VERSION}"
        cp "$daemon_stage/photon-util-${VERSION}" "$final_output_dir/$UTIL_NAME" 2>/dev/null || warn "Missing Linux util tool artifact: $daemon_stage/photon-util-${VERSION}"
    fi
...
QRC_EOF
    ...
    if [[ "$target" == "daemon" || "$target" == "both" ]]; then
        ...
        relink_native_linux_target "photon-wallet" "$SCRIPT_DIR/src/photon-wallet"
        relink_native_linux_target "photon-util" "$SCRIPT_DIR/src/photon-util"
    fi
    ...
    if [[ "$target" == "daemon" || "$target" == "both" ]]; then
        strip src/photond src/photon-cli src/photon-tx src/photon-wallet src/photon-util 2>/dev/null || true
    fi
    ...
    if [[ "$target" == "daemon" || "$target" == "both" ]]; then
        cp "$SCRIPT_DIR/src/photon-wallet" "$final_output_dir/$WALLET_NAME" 2>/dev/null || warn "Missing Linux wallet tool artifact: $SCRIPT_DIR/src/photon-wallet"
        cp "$SCRIPT_DIR/src/photon-util" "$final_output_dir/$UTIL_NAME" 2>/dev/null || warn "Missing Linux util tool artifact: $SCRIPT_DIR/src/photon-util"
    fi
...
QRC_EOF
    ...
    if [[ "$target" == "daemon" || "$target" == "both" ]]; then
        ...
        cp src/photon-wallet "$output_dir/photon-wallet-${VERSION}"
        cp src/photon-util "$output_dir/photon-util-${VERSION}"
...
PY
    ...
    # Photon 0.25.2 does not ship the upstream translation payloads.
    if [[ -f src/Makefile ]]; then
        sedi 's/^QT_QM.*=.*/QT_QM =/' src/Makefile
        sedi '/bitcoin_.*\.qm/d' src/Makefile
        sedi '/locale\/.*\.qm/d' src/Makefile
