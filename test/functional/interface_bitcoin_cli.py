...
# Generate one spendable coinbase output. Photon rewards are dynamic, so wallet
# balance checks below compare against wallet RPC results instead of hardcoding a
...
class TestBitcoinCli(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        if self.is_specified_wallet_compiled():
            ...
            assert_equal(Decimal(cli_get_info['Balance']), wallet_info['balance'])
            ...
            assert_equal(Decimal(cli_get_info['Transaction fee rate (-paytxfee) (PHO/kvB)']), wallet_info['paytxfee'])
            assert_equal(Decimal(cli_get_info['Min tx relay fee rate (PHO/kvB)']), network_info['relayfee'])
            assert_equal(self.nodes[0].cli.getwalletinfo(), wallet_info)

            # Setup to test -getinfo, -generate, and -rpcwallet= with multiple wallets.
            wallets = [self.default_wallet_name, 'Encrypted', 'secret']
            send_amounts = {wallets[1]: Decimal(9), wallets[2]: Decimal(31)}
            ...
            w1.sendtoaddress(w2.getnewaddress(), send_amounts[wallets[1]])
            w1.sendtoaddress(w3.getnewaddress(), send_amounts[wallets[2]])

            # Mine a block to confirm; adds a Photon block reward to the default wallet.
            self.generate(self.nodes[0], 1)
            expected_balances = {
                wallets[0]: w1.getwalletinfo()['balance'],
                wallets[1]: w2.getwalletinfo()['balance'],
                wallets[2]: w3.getwalletinfo()['balance'],
            }

            self.log.info("Test -getinfo with multiple wallets and -rpcwallet returns specified wallet balance")
            for wallet in wallets:
                cli_get_info_string = self.nodes[0].cli('-getinfo', f'-rpcwallet={wallet}').send_cli()
                cli_get_info = cli_get_info_string_to_dict(cli_get_info_string)
                assert 'Balances' not in cli_get_info_string
                assert_equal(cli_get_info["Wallet"], wallet)
                assert_equal(Decimal(cli_get_info['Balance']), expected_balances[wallet])
            ...
            for wallet in wallets:
                assert_equal(Decimal(cli_get_info['Balances'][wallet]), expected_balances[wallet])
            ...
            assert_equal(Decimal(cli_get_info['Balance']), expected_balances[wallets[1]])
