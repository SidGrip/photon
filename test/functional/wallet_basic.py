...
class WalletTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        node0_coinbase_value = walletinfo['immature_balance']
        assert node0_coinbase_value > 0
        assert_equal(walletinfo['balance'], 0)

        self.sync_all(self.nodes[0:3])
        self.generate(self.nodes[1], COINBASE_MATURITY + 1, sync_fun=lambda: self.sync_all(self.nodes[0:3]))

        node1_coinbase_value = self.nodes[1].getbalance()
        assert_equal(self.nodes[0].getbalance(), node0_coinbase_value)
        assert node1_coinbase_value > 0
        ...
        assert_equal(txout['value'], node0_coinbase_value)
        txout = self.nodes[0].gettxout(txid=confirmed_txid, n=confirmed_index, include_mempool=True)
        assert_equal(txout['value'], node0_coinbase_value)

        # Send 21 PHO from 0 to 2 using sendtoaddress call.
        ...
        node0_balance_after_spends = self.nodes[0].getbalance()
        self.generate(self.nodes[0], 1, sync_fun=lambda: self.sync_all(self.nodes[0:3]))
        node0_second_coinbase_value = self.nodes[0].getwalletinfo()['immature_balance']
        assert node0_second_coinbase_value > 0
        ...
        # node0 should end up with its post-send balance plus the matured
        # Photon block reward/fee output it mined above.
        assert_equal(self.nodes[0].getbalance(), node0_balance_after_spends + node0_second_coinbase_value)
        ...
        node2_balance_before_node0_sweeps = self.nodes[2].getbalance()
        node0_sweep_value = sum((utxo["amount"] - Decimal("3") for utxo in node0utxos), Decimal("0"))
        ...
        node2_balance_after_node0_sweeps = node2_balance_before_node0_sweeps + node0_sweep_value
        assert_equal(self.nodes[2].getbalance(), node2_balance_after_node0_sweeps)
        ...
        node_2_bal = self.check_fee_amount(self.nodes[2].getbalance(), node2_balance_after_node0_sweeps - Decimal('10'), fee_per_byte, self.get_vsize(self.nodes[2].gettransaction(txid)['hex']))
        assert_equal(self.nodes[0].getbalance(), Decimal('10'))

        # Send 10 PHO with subtract fee from amount
        ...
        # Sendmany 10 PHO with subtract fee from amount
        ...
        zero_output_amount = Decimal("11.11")
        zero_output_fee = Decimal("0.002")
        assert usp["amount"] > zero_output_fee
        outputs = {
            self.nodes[1].getnewaddress(): usp["amount"] - zero_output_fee,
            self.nodes[0].getnewaddress(): zero_output_amount,
        }
        ...
        if not self.options.descriptors:
            ...
            # This will raise an exception for attempting to get the private key of an invalid Photon address
            assert_raises_rpc_error(-5, "Invalid Photon address", self.nodes[0].dumpprivkey, "invalid")

            # This will raise an exception for attempting to set a label for an invalid Photon address
            assert_raises_rpc_error(-5, "Invalid Photon address", self.nodes[0].setlabel, "invalid address", "label")

            # This will raise an exception for importing an invalid address
            assert_raises_rpc_error(-5, "Invalid Photon address or script", self.nodes[0].importaddress, "invalid")
